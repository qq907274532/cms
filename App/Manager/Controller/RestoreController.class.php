...
class RestoreController extends BaseController
{
    ...
    public function import()
    {
        ...
        if ($this->_import_vol($backup['file']))
        {
            ...
            {
                $vol++;
                $link = U("Restore/import",array("vol"=>$vol,"backup"=>urlencode($this->backup_name)));
                $this->success(sprintf('导入成功！', $vol - 1), $link);
            } else
            {
                $this->success('导入成功！', U("Restore/index"));
            }
        }
    }
    ...
    public function download()
    {
        ...
        {
            header('Content-type: application/unattachmentknown');
            ...
            if ($handle = opendir(SITE_PATH . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype(SITE_PATH . $this->backup_path . $file) == 'dir')
                    {
                        ...
                        $backup['date']=$backup['date']+8*3600;
